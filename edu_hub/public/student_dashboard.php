...
// Check login - Allow viewing but show locked form if not logged in
$is_logged_in = isset($_SESSION['student_email']) && !empty($_SESSION['student_email']);
...
if ($is_logged_in) {
    ...
    // For non-logged-in users, fetch custom fields for display
    try {
        $custom_fields = $pdo->query("SELECT * FROM application_form_fields WHERE is_active = 1 ORDER BY display_order ASC")->fetchAll();
    } catch (Exception $e) {
        $custom_fields = [];
...
// Handle profile update - only if logged in
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['save_profile']) || isset($_POST['update_profile']))) {
...
// Handle application submission - only if logged in
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_app'])) {
...
// Handle send message to admin - only if logged in
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message_to_admin'])) {
...
// Handle upload additional document - only if logged in
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_additional_doc'])) {
...
// Handle contact admin - only if logged in
if ($is_logged_in && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['contact_admin'])) {
...
<head>
    ...
    <title><?php echo $is_logged_in ? 'Student Dashboard' : 'Student Application Form'; ?></title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&family=Noto+Sans+Telugu:wght@400;500;600;700&display=swap" rel="stylesheet">
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://unpkg.com/lucide@latest/dist/umd/lucide.js"></script>
    
    <!-- Tailwind Config for Navbar/Footer Colors -->
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<body>
    ...
    <?php if (!$is_logged_in): ?>
    <!-- Non-Logged In View - Show Locked Form -->
    <div class="dashboard-container">
        <div style="max-width: 900px; margin: 0 auto;">
            <!-- Page Header for Non-Logged In Users -->
            <div class="welcome-header text-center" style="justify-content: center;">
                <div style="text-align: center;">
                    <h2 style="color: var(--primary-color); margin-bottom: 10px;">
                        <i class="fas fa-file-alt me-2" style="color: var(--accent-color);"></i>
                        Student Application Form
                    </h2>
                    <p style="color: #6c757d; margin: 0;">Complete your profile and submit your application to get started</p>
                </div>
            </div>
            
            <!-- Locked Form Preview -->
            <div class="card-custom" style="position: relative; overflow: hidden; min-height: 500px;">
                <!-- Blurred Form Preview -->
                <div class="form-card-locked">
                    ...
                    <div class="card-body-custom" style="padding: 20px;">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="tel" class="form-control" placeholder="Enter contact number" disabled>
                            </div>
                            <div class="col-12 mb-3">
                                <label class="form-label fw-semibold">Address</label>
                                <textarea class="form-control" rows="2" placeholder="Enter your address" disabled></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-locked-overlay">
                    ...
                    <div class="lock-message text-center">
                        ...
                        <p>Please sign in to access the application form and submit your details.</p>
                        <a href="javascript:void(0);" onclick="openStudentLoginModal();" class="login-btn-large">
                            <i class="fas fa-sign-in-alt me-2"></i>Login to Continue
                        </a>
                        <p class="mt-3" style="font-size: 14px; color: #888;">
                            Don't have an account? <a href="/2026/edu_hub/edu_hub/student_email_register.php" style="color: var(--accent-color); font-weight: 600;">Sign up here</a>
                        </p>
                    </div>
                </div>
            </div>
            
            <!-- Information Cards for Non-Logged In Users -->
            <div class="row mt-4">
                <div class="col-md-4 mb-4">
                    <div class="card-custom h-100">
                        <div class="card-body-custom text-center py-4" style="padding: 25px;">
                            <div style="width: 60px; height: 60px; background: linear-gradient(135deg, #28a745, #5cb85c); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
                                <i class="fas fa-user-plus" style="color: white; font-size: 24px;"></i>
                            </div>
                            <h5 style="color: var(--primary-color); font-weight: 600;">Step 1: Create Account</h5>
                            <p style="color: #6c757d; font-size: 14px; margin: 0;">Sign up with your email or Google account to get started</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 mb-4">
                    <div class="card-custom h-100">
                        <div class="card-body-custom text-center py-4" style="padding: 25px;">
                            <div style="width: 60px; height: 60px; background: linear-gradient(135deg, var(--accent-color), #FFB84D); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
                                <i class="fas fa-edit" style="color: white; font-size: 24px;"></i>
                            </div>
                            <h5 style="color: var(--primary-color); font-weight: 600;">Step 2: Complete Profile</h5>
                            <p style="color: #6c757d; font-size: 14px; margin: 0;">Fill in your personal details and upload required documents</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 mb-4">
                    <div class="card-custom h-100">
                        <div class="card-body-custom text-center py-4" style="padding: 25px;">
                            <div style="width: 60px; height: 60px; background: linear-gradient(135deg, #4A90E2, #74B3F4); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
                                <i class="fas fa-paper-plane" style="color: white; font-size: 24px;"></i>
                            </div>
                            <h5 style="color: var(--primary-color); font-weight: 600;">Step 3: Submit Application</h5>
                            <p style="color: #6c757d; font-size: 14px; margin: 0;">Review and submit your application for admin approval</p>
                        </div>
                    </div>
                </div>
            </div>
